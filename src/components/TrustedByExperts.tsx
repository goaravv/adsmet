...
export const TrustedByExperts = () => {
  type Point = { name: string; CTR: number; ROAS: number };
  const initialData: Point[] = [
    { name: "Mon", CTR: 2.8, ROAS: 2.1 },
    { name: "Tue", CTR: 3.0, ROAS: 2.5 },
    { name: "Wed", CTR: 3.1, ROAS: 2.3 },
    { name: "Thu", CTR: 3.2, ROAS: 2.6 },
    { name: "Fri", CTR: 3.0, ROAS: 2.4 },
  ];
      ...
      <div className="container mx-auto px-4">
        <header className="text-center mb-8 md:mb-12">
          <h2 className="text-2xl md:text-4xl lg:text-5xl font-tech font-bold">
            ...
            <br />
            <span className="text-muted-foreground text-lg md:text-xl">Workflow</span>
          </h2>
        </header>
          ...
          <article className="neon-border bg-card/90 backdrop-blur-sm rounded-lg border border-accent/30 p-4 md:p-6 flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center gap-4 md:gap-6 mt-2">
              {workflowSteps.map((step, i) => (
                <div
                  key={step}
                  className="relative bg-muted/30 text-foreground px-5 py-3 rounded-md border border-[hsl(210,100%,56%)] animate-enter"
                  style={{ animationDelay: `${i * 150}ms` }}
                >
                  <span className="font-medium">{step}</span>
                  {i < workflowSteps.length - 1 && (
                    <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 w-px h-5 bg-foreground animate-fade-in" style={{ animationDelay: `${i * 150 + 120}ms` }} />
                  )}
                </div>
              ))}
            </div>
